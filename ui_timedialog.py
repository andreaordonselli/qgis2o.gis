...
class TreeLayerItem2(QTreeWidgetItem):
    ...
    def __init__(self, layer, tree, dlg):
        ...
        if layer.type() == layer.VectorLayer:
            ...
            if layer.customProperty("qgis2opengis/Time from"):
                self.timeFromCombo.setCurrentIndex(int(
                    layer.customProperty("qgis2opengis/Time from")))
            ...
            if layer.customProperty("qgis2opengis/Time to"):
                self.timeToCombo.setCurrentIndex(int(
                    layer.customProperty("qgis2opengis/Time to")))
    ...
    def saveLayerTimeFromComboSettings(self, value):
        ...
        if selectedLayerCombo != "None":
            selectedLayerCombo.setCustomProperty("qgis2opengis/Time from", value)
            self.populateMinMax()

    def saveLayerTimeToComboSettings(self, value):
        global selectedLayerCombo
        if selectedLayerCombo != "None":
            selectedLayerCombo.setCustomProperty("qgis2opengis/Time to", value)
            self.populateMinMax()
    ...
    def populateMinMax(self):
        ...
        for tree_layer in tree_layers:
            layer = tree_layer.layer()
            if layer.type() == QgsMapLayer.VectorLayer:
                if layer.customProperty("qgis2opengis/Time from") is not None and layer.customProperty("qgis2opengis/Time to") is not None and layer.customProperty("qgis2opengis/Time from") is not QPyNullVariant and layer.customProperty("qgis2opengis/Time to") is not QPyNullVariant:
                    for feat in layer.getFeatures():
                        attrs = feat.attributes()
                        attr = self.dateToInt(str(attrs[int(layer.customProperty("qgis2opengis/Time from")) - 1]))
                        if attr < min:
                            min = attr
                        attr2 = self.dateToInt(str(attrs[int(layer.customProperty("qgis2opengis/Time to")) - 1]))
                        if attr2 > max:
                            max = attr2
        projectInstance.writeEntry("qgis2opengis", "Min", self.dateIntToString(min))
        projectInstance.writeEntry("qgis2opengis", "Max", self.dateIntToString(max))
        #print min
        #print max
        #TODO add text boxes
...
class Button(QPushButton):
    def __init__(self, parent, main):
        ...
        icon.addPixmap(QtGui.QPixmap(
                _fromUtf8(":/plugins/qgis2opengis/icons/qgis2opengis.png")),
                QtGui.QIcon.Normal,
                QtGui.QIcon.Off)
    ...
    def saveMap(self):
        #if projectInstance.readEntry("qgis2opengis", "mapFormat")[0] == "leaflet":
        #    self.saveLeafletMap()
        #else:
        #    self.saveOLMap()
    ...
    def saveLeafletMap(self):
        # print "Save leaflet"
        dir = projectInstance.readEntry("qgis2opengis", "Exportfolder")[0]
        # print dir
    ...
    def addLeafletHeader(self, html):
        mintime = projectInstance.readEntry("qgis2opengis", "Min")[0]
        maxtime = projectInstance.readEntry("qgis2opengis", "Max")[0]
        ...
        for tree_layer in tree_layers:
            ...
            if layer.type() == QgsMapLayer.VectorLayer:
                if layer.customProperty("qgis2opengis/Time from") is not None and layer.customProperty("qgis2opengis/Time to") is not None and layer.customProperty("qgis2opengis/Time from") is not QPyNullVariant and layer.customProperty("qgis2opengis/Time to") is not QPyNullVariant:
                    ...
                    #print layer.customProperty("qgis2opengis/Time from")
                    field_from = layer.pendingFields()[int(layer.customProperty("qgis2opengis/Time from"))-1].name()
                    field_to = layer.pendingFields()[int(layer.customProperty("qgis2opengis/Time to"))-1].name()
    ...
    def saveOLMap(self):
        ...
        dir = projectInstance.readEntry("qgis2opengis", "Exportfolder")[0]
        if dir == "":
            if os.path.isdir("/tmp/qgis2opengis"):
                dir = "/tmp/qgis2opengis"
            if os.path.isdir("C:\\TEMP\\qgis2opengis"):
                dir = "C:\\TEMP\\qgis2opengis"
        # print dir
