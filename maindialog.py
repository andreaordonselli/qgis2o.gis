...
from . import utils
from qgis2opengis.configparams import (getParams,
                                   specificParams,
                                   specificOptions)
from qgis2opengis.olwriter import OpenLayersWriter
from qgis2opengis.leafletWriter import LeafletWriter
from qgis2opengis.mapboxWriter import MapboxWriter
from qgis2opengis.writerRegistry import (WRITER_REGISTRY)
from qgis2opengis.exporter import (EXPORTER_REGISTRY)
from qgis2opengis.feedbackDialog import FeedbackDialog
...
class MainDialog(QDialog, FORM_CLASS):
    """The main dialog of qgis2opengis plugin."""
    items = {}
    ...
    def __init__(self, iface, parent=None):
        ...
        self.previewFeatureLimit.setText(
            stgs.value("qgis2opengis/previewFeatureLimit", "1000"))
    ...
    def showErrorMessage(self, error):
        ...
        html += "<p>qgis2opengis produced an error:</p><code>"
        html += error
        html += "</code></body></html>"
        if self.preview:
        ...
        try:
            for layer, pop, vis, int in zip(layers, popup, visible,
                ...
                for attr in pop:
                    ...
                    layer.setCustomProperty("qgis2opengis/popup/" + attr,
                                            pop[attr])
                layer.setCustomProperty("qgis2opengis/Visible", vis)
                layer.setCustomProperty("qgis2opengis/Interactive", int)
        except Exception:
            pass

        QSettings().setValue(
            "qgis2opengis/MainDialogGeometry", self.saveGeometry())

        QSettings().setValue("qgis2opengis/previewOnStartup",
                             self.previewOnStartup.checkState())
        QSettings().setValue("qgis2opengis/closeFeedbackOnSuccess",
                             self.closeFeedbackOnSuccess.checkState())
        QSettings().setValue("qgis2opengis/previewFeatureLimit",
...
class TreeLayerItem(QTreeWidgetItem):
    ...
    def __init__(self, iface, layer, tree, dlg):
        ...
        if layer.type() == layer.VectorLayer:
            if layer.providerType() == 'WFS':
                ...
                if layer.customProperty("qgis2opengis/Encode to JSON") == 2:
            ...
            if layer.geometryType() == QgsWkbTypes.PointGeometry:
                ...
                if layer.customProperty("qgis2opengis/Cluster") == 2:
                ...
                if (custProp != "" and custProp is not None):
                    self.attrWidget.setCurrentIndex(
                        self.attrWidget.findText(
                            layer.customProperty("qgis2opengis/popup/" + option)))
            ...
            if layer.providerType() == 'wms':
                ...
                if layer.customProperty("qgis2opengis/GetFeatureInfo") == 2:
    ...
    def changeJSON(self, isJSON):
        self.layer.setCustomProperty("qgis2opengis/Encode to JSON", isJSON)

    def changeCluster(self, isCluster):
        self.layer.setCustomProperty("qgis2opengis/Cluster", isCluster)

    def changeGetFeatureInfo(self, isGetFeatureInfo):
        self.layer.setCustomProperty("qgis2opengis/GetFeatureInfo",
                                     isGetFeatureInfo)
