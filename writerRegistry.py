...
from qgis2opengis.configparams import getDefaultParams
...
class WriterRegistry(object):
    ...
    def saveTypeToProject(self, type):
        ...
        """
        assert QgsProject.instance().writeEntry("qgis2opengis", "mapFormat", type)
    ...
    def getWriterFactoryFromProject(self):
        ...
        """
        try:
            type = QgsProject.instance().readEntry("qgis2opengis",
                                                   "mapFormat")[0]
        ...
        for group, settings in params.items():
            for param, value in settings.items():
                ...
                if isinstance(value, bool):
                    QgsProject.instance().writeEntryBool("qgis2opengis",
                                                         self.sanitiseKey(
                                                             param),
                                                         value)
                else:
                    QgsProject.instance().writeEntry("qgis2opengis",
    ...
    def readParamFromProject(self, parameter, default_value):
        ...
        if isinstance(default_value, bool):
            if project.readBoolEntry(
                    "qgis2opengis", key_string)[1]:
                value = project.readBoolEntry("qgis2opengis",
                                              key_string)[0]
        elif isinstance(default_value, int):
            if project.readNumEntry(
                    "qgis2opengis", key_string)[1]:
                value = project.readNumEntry("qgis2opengis",
                                             key_string)[0]
        else:
            if (isinstance(project.readEntry("qgis2opengis",
                                             key_string)[0], str) and
                    project.readEntry("qgis2opengis", key_string)[0] != ""):
                value = project.readEntry("qgis2opengis", key_string)[0]
    ...
    def saveWriterToProject(self, writer):
        ...
        """
        QgsProject.instance().removeEntry("qgis2opengis", "/")
