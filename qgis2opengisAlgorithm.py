...
/***************************************************************************
 Process2Web
                                 A QGIS plugin
 Processing plugin for qgis2opengis
                              -------------------
        begin                : 2017-04-03
...
class qgis2opengisAlgorithm(QgsProcessingAlgorithm):
...
class exportProject(qgis2opengisAlgorithm):
    ...
    def getLayersAndGroups(self):
                ...
                try:
                    ...
                    if layer.type() == QgsMapLayer.VectorLayer:
                        for field in layer.fields():
                            fieldList = []
                            k = field.name()
                            cProp = "qgis2opengis/popup/" + field.name()
        ...
        cluster = []
        for layer in layers:
            visible.append(layer.customProperty("qgis2opengis/Visible", True))
            json.append(layer.customProperty("qgis2opengis/Encode to JSON", True))
            cluster.append(layer.customProperty("qgis2opengis/Cluster", 0) == 2)
...
class exportLayer(qgis2opengisAlgorithm):
    """This is an example algorithm that takes a vector layer and
    creates a new one just with just those features of the input
    layer that are selected.
