...
class FolderExporter(Exporter):
    ...
    def readFromProject(self):
        folder, ok = QgsProject.instance().readEntry("qgis2opengis",
                                                     "Exportfolder")
        if ok and folder:
            self.folder = folder
...
class FtpExporter(Exporter):
    ...
    def newTempFolder(self, base):
        ...
        return os.path.join(base, 'qgis2opengis_' + stamp)
    ...
    def readFromProject(self):
        host, ok = QgsProject.instance().readEntry("qgis2opengis",
                                                   "FtpHost")
        if ok and host:
            self.host = host
        user, ok = QgsProject.instance().readEntry("qgis2opengis",
                                                   "FtpUser")
        if ok and user:
            self.username = user
        folder, ok = QgsProject.instance().readEntry("qgis2opengis",
        ...
        port, ok = QgsProject.instance().readNumEntry("qgis2opengis",
                                                      "FtpPort")
        if ok and port:
            self.port = port
...
class ExporterRegistry(QObject):
    ...
    def createFromProject(self):
        ...
        :return: new exporter
        """
        type, ok = QgsProject.instance().readEntry("qgis2opengis",
                                                   "Exporter")
        exporter = None
        if ok and type:
            try:
